# 2025.12.25-Changed for implementation of MoRAgent
    ...
    **Attributes**:
    ...
    def _create_new_module(self, moelora_config, adapter_name, target):
        ...
        if loaded_in_8bit and isinstance(target, bnb.nn.Linear8bitLt):
            ...
            new_module = LinearMoELora(adapter_name, in_features, out_features, bias=bias, **kwargs)
...
class MoeLoraLayer:
    ...
    def update_layer(self, adapter_name, r, num_moe, lora_alpha, lora_dropout, init_lora_weights):
        ...
        if r > 0:
            lora_A = nn.Parameter(torch.zeros(int(self.r_num + self.e_num + self.s_num), self.in_features, 16))
            lora_B = nn.Parameter(torch.zeros(int(self.r_num + self.e_num + self.s_num), 16, self.out_features))

            # ---------input_gate----------#
            # self.gates = nn.ModuleList([nn.Linear(self.in_features, 4, bias=False) for _ in range(3)])
            # ---------input_gate----------#

            # ---------output_gate----------#
            self.lora_gate_r = nn.Linear(self.in_features, self.r_num - 1, bias=False)
            self.lora_gate_e = nn.Linear(self.in_features, self.e_num - 1, bias=False)
            self.lora_gate_s = nn.Linear(self.in_features, self.s_num - 1, bias=False)
            self.lora_gates_share = nn.ModuleList([nn.Linear(self.in_features, 1, bias=False) for _ in range(3)])
            # ---------output_gate----------#
    ...
    def reset_lora_parameters(self, adapter_name):
        ...
        # ---------output_gate----------#
        nn.init.kaiming_uniform_(self.lora_gate_r.weight, a=math.sqrt(5))
        nn.init.kaiming_uniform_(self.lora_gate_e.weight, a=math.sqrt(5))
        nn.init.kaiming_uniform_(self.lora_gate_s.weight, a=math.sqrt(5))
        for i in range(3):
            nn.init.kaiming_uniform_(self.lora_gates_share[i].weight, a=math.sqrt(5))
        # ---------output_gate----------#
...
class LinearMoELora(nn.Linear, MoeLoraLayer):
    # Lora implemented in a dense layer
    def __init__(
            self,
            adapter_name: str,
    ...
    def forward(self, x: torch.Tensor, mask_moe):
        previous_dtype = x.dtype
        batch_size, seq_len, _ = x.size()
        ...
        if self.disable_adapters:
            ...
            router_logits1 = self.lora_gate_r(x)  # result
            router_logits2 = self.lora_gate_e(x)
            router_logits3 = self.lora_gate_s(x)
            routing_weights1 = F.softmax(router_logits1, dim=2, dtype=torch.float)
            routing_weights2 = F.softmax(router_logits2, dim=2, dtype=torch.float)
            routing_weights3 = F.softmax(router_logits3, dim=2, dtype=torch.float)

            top3_values, top3_indices = torch.topk(routing_weights1, k=3, dim=-1)
            zero_mask = torch.zeros_like(routing_weights1)
            routing_weights1 = zero_mask.scatter(dim=-1, index=top3_indices, src=top3_values)
            row_sums = routing_weights1.sum(dim=-1, keepdim=True)
            routing_weights1 = routing_weights1 / row_sums
            routing_weights1 = routing_weights1.to(x.dtype)
            # print('routing_weights1 ', routing_weights1[0][327])

            top3_values, top3_indices = torch.topk(routing_weights2, k=3, dim=-1)
            zero_mask = torch.zeros_like(routing_weights2)
            routing_weights2 = zero_mask.scatter(dim=-1, index=top3_indices, src=top3_values)
            row_sums = routing_weights2.sum(dim=-1, keepdim=True)
            routing_weights2 = routing_weights2 / row_sums
            routing_weights2 = routing_weights2.to(x.dtype)
            # print('routing_weights2 ', routing_weights2[0][327])

            top2_values, top2_indices = torch.topk(routing_weights3, k=2, dim=-1)
            zero_mask = torch.zeros_like(routing_weights3)
            routing_weights3 = zero_mask.scatter(dim=-1, index=top2_indices, src=top2_values)
            row_sums = routing_weights3.sum(dim=-1, keepdim=True)
            routing_weights3 = routing_weights3 / row_sums
            routing_weights3 = routing_weights3.to(x.dtype)
            # print('routing_weights3 ', routing_weights3[0][327])

            shared_router_logits1 = F.sigmoid(self.lora_gates_share[0](x))
            shared_router_logits2 = F.sigmoid(self.lora_gates_share[1](x))
            shared_router_logits3 = F.sigmoid(self.lora_gates_share[2](x))

            routing_weights = torch.cat(
                (routing_weights1, shared_router_logits1, routing_weights2, shared_router_logits2, routing_weights3,
                 shared_router_logits3), dim=2)
            # print('routing_weights ', routing_weights[0][327])
            mask_repeat1 = mask_moe[:, :, 0].unsqueeze(2).repeat(1, 1, self.r_num)
            mask_repeat2 = mask_moe[:, :, 1].unsqueeze(2).repeat(1, 1, self.e_num)
            mask_repeat3 = mask_moe[:, :, 2].unsqueeze(2).repeat(1, 1, self.s_num)
            mask_repeat = torch.cat((mask_repeat1, mask_repeat2, mask_repeat3), dim=2) * routing_weights
            out = torch.einsum('bned,bne->bned', out, mask_repeat)
            out = out.sum(dim=2)
            result = result + out
            # ---------多个低秩实现+output_gate----------#
        else:
        ...
        indices = [i for i in range(14) if i not in [4, 9, 13]]
        new_routing_weights = routing_weights[:, :, indices]
        return result, new_routing_weights