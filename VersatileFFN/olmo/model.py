# 2025.12.17-Changed for implementation of VersatileFFN
...
class OLMoBlock(nn.Module):
    ...
    def build(cls, layer_id: int, config: ModelConfig, cache: BufferCache) -> OLMoBlock:
        ...
        elif config.block_type == BlockType.versatile:
            return OLMoVersatileBlock(layer_id, config, cache)
        else:
...
class OLMoVersatileBlock(OLMoBlock):
    def __init__(self, layer_id: int, config: ModelConfig, cache: BufferCache):
        super().__init__(layer_id, config, cache)
        head_dim = config.d_model // config.n_heads
        self.fused_dims = (
        ...
        self.max_depth = 4  
        self.expert_gate = nn.Linear(config.d_model, self.total_experts, bias=False, device=config.init_device)
        self.depth_predictor = nn.Linear(config.d_model, self.max_depth, bias=False, device=config.init_device)
        self.attn_norm = LayerNorm.build(config, size=config.d_model)
        self.ff_norm = LayerNorm.build(config, size=config.d_model)
        self.att_proj = nn.Linear(config.d_model, sum(self.fused_dims), bias=config.include_bias, device=config.init_device)
        self.ff_proj = nn.Linear(config.d_model, self.hidden_size, bias=config.include_bias, device=config.init_device) 
        self.ff_out = nn.Linear(int(self.act.output_multiplier * self.hidden_size), config.d_model,bias=config.include_bias, device=config.init_device)

        self.d_model = config.d_model
        segment_size = int(self.hidden_size * self.act.output_multiplier / self.total_experts)
        B_idx_list = [torch.arange(i * segment_size, (i + 1) * segment_size) for i in range(self.total_experts)]
        tmp_idx_list = [B_idx_list[i] + self.hidden_size//2 for i in range(self.total_experts)]
        A_idx_list = [torch.cat((b, tmp), dim=0) for b, tmp in zip(B_idx_list, tmp_idx_list)]
        self.B_idx_tensor = torch.stack(B_idx_list)
        self.A_idx_tensor = torch.stack(A_idx_list)

        self.gumbel_temp_start = 5.0   
        self.gumbel_temp_end = 0.1     
        self.gumbel_temp_decay = 0.99984
        self.current_temp = self.gumbel_temp_start

    def update_temperature(self):
        self.current_temp = max(self.current_temp * self.gumbel_temp_decay, self.gumbel_temp_end)
    ...
    def reset_parameters(self):
        ...
        if self.config.init_fn == InitFnType.normal:
            std = ff_out_std = self.config.init_std
            cutoff_factor = self.config.init_cutoff_factor
        elif self.config.init_fn == InitFnType.mitchell:
            std = 1 / math.sqrt(self.config.d_model)
            ff_out_std = 1 / (math.sqrt(2 * self.ff_out.in_features * (self.layer_id + 1)))
        ...
        init_normal(self.att_proj, std, cutoff_factor)
        init_normal(self.expert_gate, std, cutoff_factor)
        init_normal(self.depth_predictor, std, cutoff_factor)
    ...
    def forward(
        ...
        if self.config.clip_qkv is not None:
            qkv.clamp_(min=-self.config.clip_qkv, max=self.config.clip_qkv)
        q, k, v = qkv.split(self.fused_dims, dim=-1)
        att, cache_out = self.attention(q, k, v, attention_bias, layer_past=layer_past, use_cache=use_cache, max_doc_len=max_doc_len, cu_doc_lens=cu_doc_lens)
        x_out = x + self.dropout(att)
        bs, seq_len, _ = x_out.shape

        depth_logits = self.depth_predictor(x_out)  # [bs, seq, max_depth]

        # ============ Training ============ #
        if self.training:
            depth_probs = F.gumbel_softmax(depth_logits, tau=self.current_temp, hard=True, dim=-1)
            # ============ Depth-Versatile ============ #
            depth_outputs = []
            current_x = x_out
            for _ in range(self.max_depth):
                og_x = current_x
                current_x = self.ff_norm(current_x)
                current_x = self.ff_proj(current_x)
                current_x = self.act(current_x)
                current_x = self.ff_out(current_x)
                current_x = self.dropout(current_x)
                current_x = og_x + current_x
                depth_outputs.append(current_x)
            
            depth_outputs = torch.stack(depth_outputs, dim=-1)
            depth_probs_exp = depth_probs.unsqueeze(-2)
            x_depth = (depth_outputs * depth_probs_exp).sum(dim=-1)

            # ============ Width-Versatile ============ #
            routing_logits = self.expert_gate(x_out)
            topk_weights, topk_indices = torch.topk(routing_logits, k=self.active_experts, dim=-1)
            topk_weights = torch.softmax(topk_weights, dim=-1)
            
            x_flat = x_out.view(-1, self.d_model)
            topk_indices_flat = topk_indices.view(-1, self.active_experts)
            topk_weights_flat = topk_weights.view(-1, self.active_experts)
            x_moe_flat = torch.zeros_like(x_flat)
            
            for expert_idx in range(self.total_experts):
                expert_mask = (topk_indices_flat == expert_idx)
                token_indices, k_indices = torch.where(expert_mask)
                if len(token_indices) == 0: 
                    continue
                
                select_a_w = self.ff_proj.weight[self.A_idx_tensor[expert_idx], :]
                select_b_w = self.ff_out.weight[:, self.B_idx_tensor[expert_idx]]
                expert_input = x_flat[token_indices]
                expert_weights = topk_weights_flat[token_indices, k_indices].unsqueeze(-1)
                
                h = self.ff_norm(expert_input)
                h = h @ select_a_w.T
                h = self.act(h)
                h = h @ select_b_w.T
                h = self.dropout(h)
                ...
                weighted_h = (h * expert_weights).to(x_moe_flat.dtype)
                x_moe_flat.index_add_(0, token_indices, weighted_h)
            
            x_moe = x_moe_flat.view(bs, seq_len, self.d_model)
            
            # ============ Fusion Weight ============ #
            loop_indices = torch.arange(1, self.max_depth + 1, device=x.device, dtype=depth_probs.dtype)
            expected_loops = (depth_probs * loop_indices.view(1, 1, -1)).sum(dim=-1, keepdim=True)
            moe_weight = (self.max_depth - expected_loops) / self.max_depth

        # ============ Inference ============ #
        else:
            loop_choice = depth_logits.argmax(dim=-1)  # [bs, seq] (0 ~ max_depth-1)
            
            # ============ Depth-Versatile ============ #
            max_batch_loop = loop_choice.max().item()
            
            depth_outputs = []
            current_x = x_out

            for loop_idx in range(max_batch_loop + 1):
                og_x = current_x
                current_x = self.ff_norm(current_x)
                current_x = self.ff_proj(current_x)
                current_x = self.act(current_x)
            ...
            depth_outputs = torch.stack(depth_outputs, dim=-1)  # [bs, seq, d_model, depth_run]
            gather_indices = loop_choice.unsqueeze(-1).unsqueeze(-1).expand(bs, seq_len, self.d_model, 1)
            x_depth = depth_outputs.gather(dim=-1, index=gather_indices).squeeze(-1)

            # ============ Fusion Weight ============ #
            expected_loops = (loop_choice + 1).float() 
            moe_weight = (self.max_depth - expected_loops) / self.max_depth
            moe_weight = moe_weight.unsqueeze(-1) # [bs, seq, 1]

            # ============ Width-Versatile ============ #
            active_mask = (moe_weight.squeeze(-1) > 1e-6) # [bs, seq]
            x_moe = torch.zeros_like(x_out)
            if active_mask.any():
                x_flat_all = x_out.view(-1, self.d_model)
                active_mask_flat = active_mask.view(-1)
                
                x_active = x_flat_all[active_mask_flat]
                
                routing_logits_active = self.expert_gate(x_active)
                topk_weights, topk_indices = torch.topk(routing_logits_active, k=self.active_experts, dim=-1)
                topk_weights = torch.softmax(topk_weights, dim=-1)
                
                x_moe_active = torch.zeros_like(x_active)
                
                topk_indices_flat = topk_indices # [num_active, k]
                topk_weights_flat = topk_weights # [num_active, k]
                ...
                for expert_idx in range(self.total_experts):
                    ...
                    expert_input = x_active[token_indices]
                    expert_weights = topk_weights_flat[token_indices, k_indices].unsqueeze(-1)
                    ...
                    weighted_h = (h * expert_weights).to(x_moe_active.dtype)
                    x_moe_active.index_add_(0, token_indices, weighted_h)
                
                x_moe_flat = x_moe.view(-1, self.d_model)
                x_moe_flat[active_mask_flat] = x_moe_active
                x_moe = x_moe_flat.view(bs, seq_len, self.d_model)
                
                routing_logits = torch.zeros(bs * seq_len, self.total_experts, device=x.device, dtype=routing_logits_active.dtype)
                routing_logits[active_mask_flat] = routing_logits_active
            else:
                routing_logits = torch.zeros(bs * seq_len, self.total_experts, device=x.device, dtype=x.dtype)

        # ============ Final Fusion for Training and Inference ============ #
        x_final = x_depth * (1 - moe_weight) + x_moe * moe_weight

        return x_final, cache_out
