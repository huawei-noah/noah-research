...
def _generate_relative_positions_embeddings(length, depth, max_relative_position=127):
    ...
    distance_mat_clipped = torch.clamp(distance_mat, -max_relative_position, max_relative_position)
    # final_mat = distance_mat_clipped + max_relative_position
    final_mat = distance_mat_clipped + min(max_relative_position, length)
...
class NeZhaSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.relative_positions_embeddings = _generate_relative_positions_embeddings(
            ...
            #length=512, depth=self.attention_head_size, max_relative_position=config.max_relative_position).to(device)
            length=config.seq_length, depth=self.attention_head_size, max_relative_position=config.max_relative_position).to(device)
        self.dropout = nn.Dropout(config.attention_probs_dropout_prob)
