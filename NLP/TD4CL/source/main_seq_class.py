...
from trainers.seq_class_trainer import SeqClassTrainer as Trainer
...
def main(config):
    ...
    dataset = data_files.filename(config['dataset_name'])
    logger.info(dataset)

    # labels
    label_map = {d['label']: d['id'] for d in dataset['label_map']}
    dataset.pop('label_map')

    # tokenizer
    logger.info('*** Loading {} tokenizer ***'.format(config['model_name'].upper()))
    tokenizer = AutoTokenizer.from_pretrained(
        config['model_name'],
    )

    # processor
    processor = SequenceClassificationProcessor(config, tokenizer, label_map)
    processed_data = dataset.map(processor.convert_to_features, batched=True)

    if args.show_examples and args.mode == 'train':
        show_random_examples(dataset['train'], num_examples=10, rand=True)

    # show_random_examples(dataset['test'], num_examples=10, rand=True)
    ########################
    # Set up Trainer
    ########################
    ...
    trainer = curric_trainer(config, processor, datasets=processed_data, raw_data=dataset)
    ...
    if 'train' in args.mode:
        ...
        elif config['curriculum'] == 'baby-step':
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_patience', type=int, default=25, help='Patience (# evaluations) before stopping')
    ...
    parser.add_argument('--log_interval', type=int, help='Log per # batches')
    ...
    parser.add_argument('--steps', type=int, help='Stop at the indicated number of steps')
