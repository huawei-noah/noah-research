...
# from ...run_distill import parse_single_batch
...
class DataLoaderIter(object):
  ...
  def __next__(self):
    ...
    '''
    return next(self._iterator) # to be `parse_single_batch`
...
class TrainDataLoaderIter(DataLoaderIter):
  ...
  def __next__(self):
    ...
    '''
    try:
      return super(TrainDataLoaderIter, self).__next__()
    except StopIteration:
      if not self.auto_reset:
        raise
      self._iterator = iter(self.data_loader)
...
class LRFinder(object):
  ...
  def __init__(
    ...
    self.model_type_t = model_type_t
    self.model_type = model_type
    self.sent_cls = sent_cls
    self.loss_kl_temp = loss_kl_temp
    self.task_type = task_type # 211210
  ...
  def range_test_distill( # 211118
    ...
    for iteration in range(num_iter):
      ...
      lr_schedule.step()
      print(f"  [range_test_distill] step = {iteration}, lr = {self.history['lr'][-1]}, loss = {loss}")
      sys.stdout.flush()
  ...
  def _train_batch(self, train_iter, accumulation_steps, non_blocking_transfer=True):
    ...
    for i in range(accumulation_steps):
      ...
      batch_t, batch_s = next(train_iter)
      batch_t, batch_s = tuple(t.to(self.device) for t in batch_t), tuple(t.to(self.device) for t in batch_s)
      if self.task_type == "s_cls":
        inputs_t = parse_single_batch(batch_t, self.model_type_t, self.sent_cls) # 211118
        inputs = parse_single_batch(batch_s, self.model_type, self.sent_cls) # 211118
      elif self.task_type == "seq_tag": # 211202
        inputs_t = parse_single_batch_tag(batch_t, self.model_type_t)
        inputs = parse_single_batch_tag(batch_s, self.model_type)
      # inputs, labels = self._move_to_device(
      #   inputs, labels, non_blocking=non_blocking_transfer
      # )

      # Forward pass
      outputs_t = self.model_t(**inputs_t)
      outputs = self.model(**inputs)
      logits_t = outputs_t[1]
      logits = outputs[1]
      dist_t = F.softmax(logits_t / self.loss_kl_temp, dim=-1) # [B, C]
      dist = F.softmax(logits / self.loss_kl_temp, dim=-1)
      if self.task_type == "s_cls":
        loss = self.criterion(dist.log(), dist_t)
      elif self.task_type == "seq_tag": # 211202 
        dist_pos_t = batch_t[4] # add `dist_pos` field. [B, T] w/ 0-paddings
        dist_pos_s = batch_s[4]
        if self.model_type_t == "xlm": dist_pos_t = batch_t[5]
        if self.model_type == "xlm": dist_pos_s = batch_s[5]
        ...
        loss = self.criterion(dist.log() * dist_mask_s, dist_t * dist_mask_t)
  ...
  def _validate(self, val_iter, non_blocking_transfer=True):
    ...
    with torch.no_grad():
      for batch in val_iter:
        ...
        if self.task_type == "s_cls":
          inputs = parse_single_batch(batch, self.model_type, self.sent_cls) # 211118
        elif self.task_type == "seq_tag": # 211202
          inputs = parse_single_batch_tag(batch, self.model_type)
        outputs = self.model(**inputs)
        tmp_eval_loss, logits = outputs[:2]
        eval_loss += tmp_eval_loss.mean().item()
        # nb_eval_steps += 1
      
    return eval_loss / len(val_iter.dataset)
  ...
  def plot(
    ...
    print(f"  suggest_lr = {suggest_lr}, min_grad_idx = {min_grad_idx}")
    sys.stdout.flush()
