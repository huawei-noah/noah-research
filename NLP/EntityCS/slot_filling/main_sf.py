...
def main():
    ...
    # unique labels.
    def get_all_label_list(labels_list):
        unique_labels = set()
    ...
    if isinstance(features[label_column_name].feature, ClassLabel):
        ...
        all_label_data = [raw_datasets["train"][label_column_name]]
        if model_args.all_val_langs:
            for lang in model_args.eval_languages.split(","):
                all_label_data.append(
                    raw_datasets["validation"][lang][label_column_name]
                )
        else:
            all_label_data.append(raw_datasets["validation"][label_column_name])
        for lang in model_args.eval_languages.split(","):
            all_label_data.append(raw_datasets["test"][lang][label_column_name])
        label_list = get_all_label_list(all_label_data)
    ...
    def tokenize_and_align_labels(examples):
        ...
        for i, slot_label in enumerate(examples[label_column_name]):
            ...
            for word_idx, token_idx in zip(word_ids, token_ids):
                ...
                # We set the label for the first token of each word.
                    ...
                    label_ids.append(label_to_id[slot_label[word_idx]])
                ...
                # the label_all_tokens flag.
                    ...
                    if data_args.label_all_tokens:
                        label_ids.append(
                            b_to_i_label[label_to_id[slot_label[word_idx]]]
                        )
                    else:
                        label_ids.append(-100)
        ...
        tokenized_inputs["labels"] = slot_labels
