...
from bo.acquisitions.acq import Mean, Sigma, MOMeanSigmaLCB
        ...
        if self.X.shape[0] < 2 * self.space.num_paras:
            ...
            try:
                if self.y.min() <= 0:
                    ...
                    if y.std() < 0.5:
                        y = torch.FloatTensor(power_transform(self.y / self.y.std(), method='yeo-johnson'))
                model = GP(self.space.num_numeric, self.space.num_categorical, 1, lr=0.03, num_epochs=100, verbose=True,
                           pred_likeli=False)
                model.fit(X, Xe, y)
                print('Output scale: %g,  noise: %g %g' % (model.gp.kern.outputscale, model.lik.noise, model.noise),
                      flush=True)
            except:
                ...
                y = y[filt]
                model = GP(self.space.num_numeric, self.space.num_categorical, 1, lr=0.03, num_epochs=100, verbose=True,
                           pred_likeli=False, noise_lb=8e-4)
                model.fit(X, Xe, y)
