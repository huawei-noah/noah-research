...
class GeneralBO(AbstractOptimizer):
    ...
    def suggest(self, n_suggestions=1, fix_input=None):
        ...
        if self.X.shape[0] < self.rand_sample:
            ...
            num_uniqs = None if Xe.shape[1] == 0 else [len(self.space.paras[name].categories) for name in
                                                       self.space.enum_names]
            self.model = get_model(self.model_name, X.shape[1], Xe.shape[1], y.shape[1], num_uniqs=num_uniqs,
                                   **self.model_conf)
            self.model.fit(X, Xe, y)
            
            acq = GeneralAcq(self.model,
                             self.num_obj,
                             self.num_constr,
                             kappa=self.kappa,
