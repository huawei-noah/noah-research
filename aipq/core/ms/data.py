...
def img2tensor(imgs, bgr2rgb=True, float32=True):
    ...
    def _totensor(img, bgr2rgb, float32):
        ...
        if float32:
            img = mindspore.Tensor(img.copy().transpose(2, 0, 1),dtype=mindspore.float32)
        else:
            img = mindspore.Tensor(img.copy().transpose(2, 0, 1))
        return img

    _totensorPIL = vision.ToTensor()
...
class TrainTransforms(object):
    def __init__(self, normalize=None, patch_size=64,
        ...
        if normalize is None:
            self.normalize = vision.Normalize(
    ...
    def __call__(self, imgs):
        ...
        # normalize first, and then convert to MS tensor
...
class ValTransforms(object):
    ...
    def __call__(self, imgs):
        ...
        imgs = img2tensor(imgs, bgr2rgb=self.bgr2rgb)
        imgs = [mindspore.Tensor(self.normalize(img.asnumpy())) for img in imgs]
        return imgs
...
class PIPALsinglewRef():
    def __init__(self, root, transform,
        ...
        df = pd.read_csv(df_path, index_col=0)  # all other datasets, this one (including dataset=ntire2021)
    ...
    def __getitem__(self, index):
        ...
        elo = mindspore.Tensor(sample['elo'],dtype=mindspore.float64)
...
class IQAsinglewRef:
    ...
    def __init__(self, root, transform,
        ...
        # maybe add here one more option: mindspore record format
    ...
    def __getitem__(self, index):
        ...
        mos = mindspore.Tensor(sample['mos'],dtype=mindspore.float64)
