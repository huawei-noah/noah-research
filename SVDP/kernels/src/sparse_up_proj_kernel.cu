...
__global__ void sparse_up_proj(__half* __restrict__ weight, __half* __restrict__ x, __half* __restrict__ gate_out, float threshold) {
...
void launch_sparse_up_proj(__half* weight, __half* x, __half* gate_out, float threshold) {

    dim3 grid_size(N_ROWS);
    dim3 block_size(NUM_THREADS);

    sparse_up_proj<<<grid_size, block_size>>>(weight, x, gate_out, threshold);
}