...
class Qwen2MLP(nn.Module):
    def __init__(
        ...
        self.svd_predictor_down_proj = RowParallelLinear(hidden_size,
                                                         predictor_rank,
                                                         bias=False,
                                                         quant_config=None)
        self.svd_predictor_up_proj = ColumnParallelLinear(predictor_rank,
                                                         intermediate_size,
                                                         bias=True,
                                                         quant_config=None)
...
class Qwen2Model(nn.Module):
    ...
    def load_weights(self, weights: Iterable[Tuple[str,
        ...
        predictors_state_dict = torch.load(self.predictors_path)
        for param_name, param in params_dict.items():
            if "svd_predictor" in param_name:
                weight_loader = param.weight_loader
                weight_loader(param, predictors_state_dict[param_name].data)
...
class Qwen2ForCausalLM(nn.Module, SupportsLoRA, SupportsPP):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        try:
            import os
            predictors_path = os.environ["SVD_PREDICTORS_PATH"]
        except Exception:
            predictors_path = config.predictors_path
        predictors_state_dict = torch.load(predictors_path, map_location='meta')
        config.ranks = [predictors_state_dict[f"layers.{i}.mlp.svd_predictor_down_proj.weight"].shape[0] for i in range(config.num_hidden_layers)]
