...
def parse_ranks_as_string(ranks_str: str):
    try:
        ...
        raise argparse.ArgumentTypeError("Invalid ranks")
    return values
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--calibration_prompts_path", help="Path to calibration prompts (json)", type=str, required=True)
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("--rank", help="Predictor rank",  type=int)
    group.add_argument("--ranks", type=parse_ranks_as_string, help="Comma-separated list of ranks (e.g., 256,256,512,...,128)")
    ...
    parser.add_argument("--sparsity_plot_output_file", type=str, required=False, default="sparsity.pdf", help="Output filename (pdf)")
    parser.add_argument("--ablation_config_whitening", choices=["w/o", "cholesky"], required=False, default="cholesky", help="Ablation")
    parser.add_argument("--ablation_config_bias", choices=["w/o", "const", "full"], required=False, default="full", help="Ablation")
    parser.add_argument("--ablation_config_penalty", choices=["gate", "gate/up", "full"], required=False, default="full", help="Ablation")
...
def collect_hidden_activations(model, tokenizer, prompts: list[str]) -> Tuple:
        ...
        for layer_id, layer in enumerate(model.model.layers):
            hook_handles.append(
                layer.mlp.gate_proj.register_forward_hook(
                    lambda module, input, output, layer_id=layer_id: gate_hook(
                        module, input, output, layer_id
                    )
                )
            )
            hook_handles.append(
                layer.mlp.down_proj.register_forward_hook(
                    lambda module, input, output, layer_id=layer_id: down_hook(
                        module, input, output, layer_id
                    )
                )
            )
    ...
    history_len = len(mlp_input_history[0])
...
def estimate_sparsity(gate_outputs, plot_output_path: Optional[str]=None) -> np.array:
...
# ablation_config = {
#     "whitening" : ("w/o", "cholesky",
#     "bias" : ("w/o", "const", "full"),
#     "penalty" : ("gate", "gate/up", "full"),
# }

def compute_predictor_weights(w_gate: torch.Tensor, w_down: torch.Tensor, r: int, mlp_input: torch.Tensor, gate_output: torch.Tensor, down_input: torch.Tensor, act_fn: torch.nn.Module, desired_sparsity: float, ablation_config:dict):
    ...
    if ablation_config["bias"]=="const":
        threshold = np.quantile(predicted_values.flatten().detach().cpu().to(torch.float32).numpy(), desired_sparsity).item()
...
def build_predictor(mlp, desired_sparsity, mlp_input, gate_output, down_input, rank, ablation_config):
    ...
    predictor = Predictor(
        hidden_size=hidden_size, intermediate_size=n_neurons, rank=rank
    ) 
...
def main() -> None:
    ...
    args = parse_args()
    try:
        model, tokenizer = load_model_and_tokenizer(args.model_path, attn_implementation="sdpa", torch_dtype=args.torch_dtype, device_map=args.device_map)
    except ValueError:
        logger.info(f"sdpa attention is not supported, rollback to eager!")
        model, tokenizer = load_model_and_tokenizer(args.model_path, attn_implementation="eager", torch_dtype=args.torch_dtype, device_map=args.device_map)
