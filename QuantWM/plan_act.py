...
import re
from quant_utils.smoothquant_utils import get_act_scales_wm
...
class PlanWorkspace:
    def __init__(
        ...
        self.calib_state = cfg_dict["sta_scale"]
...
def load_model(model_ckpt, train_cfg, cfg_dict, num_action_repeat, device):
    ...
    def _get_default_quant_cfg():

        from models.ptq.bit_type import BitType  
        class _C:
            pass
        c = _C()
        c.BIT_TYPE_W = BitType(bits=32, signed=True, name="fp32")
        c.BIT_TYPE_A = BitType(bits=32, signed=True, name="fp32")


        c.CALIBRATION_MODE_W = "layer_wise"
        c.CALIBRATION_MODE_A = "layer_wise"
        c.OBSERVER_W = "minmax"
        c.OBSERVER_A = "minmax"
        c.QUANTIZER_W = "uniform"
        c.QUANTIZER_A = "uniform"

        return c

    def _remap_predictor_sd_old_to_new(old_sd: dict) -> dict:

        if any(".attn." in k or ".ff." in k for k in old_sd.keys()):
            return old_sd
    ...
    quant_cfg = cfg_dict.get("quant_cfg_predictor", None)
    if quant_cfg is None:
        quant_cfg = _get_default_quant_cfg()
        print(
            "[load_model] quant_cfg_predictor not found in cfg_dict, "
            "using default stub cfg (FP mode)"
        )
    ...
    missing, unexpected = new_predictor.load_state_dict(
        mapped_pred_sd, strict=True
    )

    print(
        f"[load_model] predictor load done. "
        f"missing={len(missing)}, unexpected={len(unexpected)}"
    )
    if missing:
        print("[load_model] missing example:", missing[:20])
    if unexpected:
        print("[load_model] unexpected example:", unexpected[:20])

    print("[load_model] check predictor weight diff")
    ...
    for k, v in mapped_pred_sd.items():
        if k in new_sd:
            if (v - new_sd[k]).abs().sum().item() > 0:
                ...
                if diff_cnt <= 20:
                    print("diff key:", k)
    print(f"[load_model] check diff done (diff_cnt={diff_cnt})")
    ...
    if train_cfg.has_decoder and "decoder" not in result:
        ...
        if train_cfg.env.decoder_path is not None:
            ...
            raise ValueError("Decoder path not found and not provided in config")
...
def planning_main(cfg_dict):
    ...
    def name_filter(name: str):
        return name.startswith("encoder") or name.startswith("predictor")

    act_scales, hooks = get_act_scales_wm(model, name_filter=name_filter)
    ...
    print(f"[planning_main] saved act_scales to {scales_path}")
    for key, value in act_scales.items():
        print(f"the dino_wm key {key} weight shape is {value.shape}")
