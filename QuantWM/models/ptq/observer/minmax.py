...
class MinmaxObserver(BaseObserver):
    ...
    def __init__(self, module_type, bit_type, calibration_mode,shape=None):
        ...
        if module_type =='linear_weight':
            if self.group_size!=-1:
                dim1 = int(shape[0]*math.ceil(shape[1]/self.group_size))
                self.deficiency = shape[-1]%self.group_size
                if self.deficiency > 0:
                    self.deficiency = self.group_size - self.deficiency
    ...
    def get_quantization_params(self, *args, **kwargs):
        ...
        if self.module_type =='linear_weight':
            if self.lwc:
                max_val = self.sigmoid(self.upbound_factor)*max_val
                min_val = self.sigmoid(self.lowbound_factor)*min_val
