...
from .nearest_embed import NearestEmbed,SimVQ1D # NearestEmbedEMA,
...
class Mlp_VAE(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, \
                 bias=True,norm_layer=nn.LayerNorm, k=10,vq_coef=0.4, commit_coef=0.1,expender=1024,drop=0.): #0.2,0.4 commit_coef=0.01
        super().__init__()
        out_features = out_features or in_features
        hidden_features = hidden_features #or in_features
        self.enlinear=nn.Linear(in_features, hidden_features)
        self.delinear=nn.Linear(hidden_features, in_features)
        
        self.emb = NearestEmbed(k, out_features)#hidden_features
        # self.embnew=SimVQ1D(k,out_features)
        
        self.ennorm3=RecoverLN(out_features,expender)
        
        self.en_addnorm2 = AddNorm(out_features,expender)
        
        self.denorm2=RecoverLN(out_features,expender)
        
        self.de_addnorm2 = AddNorm(out_features,expender)
    ...
    def loss_function(self, x, recon_x, z_e, emb, argmin):
        ...
        self.commit_loss=torch.norm((recon_x - x)**2, 1, 1).topk(100).values.sum()
