...
@dataclass
class MultiddsMultidomainTranslationConfig(FairseqDataclass):
    ...
    sim_mat_dir: str = field(
        default="",
        metadata={
            "help": "path to store the cosine similarity matrix"
        }
    )
...
@register_task("multidds_multidomain_translation", dataclass=MultiddsMultidomainTranslationConfig)
class MultiddsMultidomainTranslation(FairseqTask):
    """
    ...
    cfg: MultiddsMultidomainTranslationConfig

    def __init__(self, cfg: MultiddsMultidomainTranslationConfig, src_dict, tgt_dict):
        super().__init__(cfg)
        self.cfg = cfg
        ...
        if not os.path.exists(self.cfg.sim_mat_dir):
            os.makedirs(self.cfg.sim_mat_dir)

    @classmethod
    def setup_task(cls, cfg: MultiddsMultidomainTranslationConfig, **kwargs):
    ...
    def train_step(
        ...
        if update_num % self.cfg.update_sampling_interval == 0 and update_num != 0 and data_actor is not None and self.current_sampling_update_num != update_num:
            ...
            p, sim_mat = self.update_sample_probability(model, criterion, data_actor, data_optimizer, prepare_fn)
            np.save(os.path.join(self.cfg.sim_mat_dir, "sim_mat_%010d.npy" % update_num), sim_mat)
    ...
    def update_sample_probability(
        ...
        for i, valid_key in enumerate(self.datasets["train"].datasets.keys()):
            ...
            loss, sample_size, logging_output = criterion(model, sample)
            loss.backward()
            valid_grad_state_i = self.copy_grad(model)
            if torch.cuda.is_available():
                torch.cuda.empty_cache()

            sim_list = []
            for i, train_key in enumerate(self.datasets["train"].datasets.keys()):
                model.zero_grad()
                sample = self.datasets["train"].get_sample_with_key(train_key)[train_key]
                sample, _ = prepare_fn(sample)
                loss, sample_size, logging_output = criterion(model, sample)
                if sample_size > 0:
                    loss = loss / sample_size
                loss.backward()
                train_grad_state_i = self.copy_grad(model)
                cos_sim = self.grad_cosine_sim(train_grad_state_i, valid_grad_state_i)
        ...
        return prob.data.view(-1).cpu().numpy(), np.array(all_sim_list)
