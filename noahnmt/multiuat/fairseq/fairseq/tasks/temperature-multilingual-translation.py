...
@register_task("temperature_multilingual_translation")
class TemperatureMultilingualTranslationTask(MultilingualTranslationTask):
    ...
    # def calc_p(self, t):
    #     sizes = np.array(list(map(len, self.datasets["train"])))
    #     q = sizes / np.sum(sizes)
    ...
    #     sizes = np.array(list(map(len, self.datasets["train"])))
    #     dic = OrderedDict()
    #     for x, y, z in list(zip(self.datasets.keys(), sizes, self.p.tolist())):
    #         dic[x] = (y, z)
    #         logger.info("Language Pair: %8s | size = %8d | Sampling probability = %6.3f%% " % (x, y, z*100))
    #     return dic
    ...
    #     return criterion(model.models[lang_pair], sample)
