...
class MultiUncertainTrainer(object):
    ...
    def __init__(self, cfg: FairseqConfig, task, model, data_actor, criterion, quantizer=None):
        ...
        if self.data_actor:
            self.data_optimizer = torch.optim.Adam([p for p in self.data_actor.parameters() if p.requires_grad], lr=self.cfg.task.data_actor_lr)
        if cfg.common.fp16:
            self._criterion = self._criterion.half()
            self._model = self._model.half()
            self.data_actor = self.data_actor.half()
        elif cfg.common.bf16:
            self._criterion = self._criterion.to(dtype=torch.bfloat16)
            self._model = self._model.to(dtype=torch.bfloat16)
            self.data_actor = self.data_actor.to(dtype=torch.bfloat16)
        if (
            ...
            self.data_actor = self.data_actor.to(device=self.device)
            ...
            try:
                with maybe_no_sync():
                    ...
                    loss, sample_size_i, logging_output = self.task.train_step(
                        ...
                        prepare_fn=self._prepare_sample
                    )
                    del loss
