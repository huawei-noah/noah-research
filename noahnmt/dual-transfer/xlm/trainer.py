...
class Trainer(object):
    ...
    def set_optimizers(self):
        ...
        if params.freeze_except_emb:
            assert 'model' in self.MODEL_NAMES
            parameters_list = [p for k, p in getattr(self, 'model').named_parameters() if p.requires_grad and not k.endswith(HashingMemory.MEM_VALUES_PARAMS) and k in {'embeddings.weight', 'pred_layer.proj.weight', 'pred_layer.proj.bias'}]
            self.optimizers['model'] = get_optimizer(parameters_list, params.optimizer)
        elif params.freeze_encoder_emb:
            assert 'encoder' in self.MODEL_NAMES
            parameters_list = [p for k, p in getattr(self, 'encoder').named_parameters() if p.requires_grad and not k.endswith(HashingMemory.MEM_VALUES_PARAMS) and k not in {'embeddings.weight', 'pred_layer.proj.weight', 'pred_layer.proj.bias'}]
            parameters_list.extend([p for k, p in getattr(self, 'decoder').named_parameters() if p.requires_grad and not k.endswith(HashingMemory.MEM_VALUES_PARAMS)])
            self.optimizers['model'] = get_optimizer(parameters_list, params.optimizer)
        else:
