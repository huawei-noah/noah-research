# 2021 - Added code for dual transfer
#        Huawei Technologies Co., Ltd. <zhangmeng92@huawei.com>
...
def build_model(params, dico, tgt_dico=None):
    """
    Build model.
    """
    if params.encoder_only:
        ...
        # reload a pretrained model except embeddings
        if params.reload_model_except_emb != '':
            assert params.reload_model == '', 'Cannot set both --reload_model and --reload_model_except_emb'
            logger.info("Reloading model from %s except embeddings ..." % params.reload_model_except_emb)
            reloaded = torch.load(params.reload_model_except_emb, map_location=lambda storage, loc: storage.cuda(params.local_rank))['model']
            if all([k.startswith('module.') for k in reloaded.keys()]):
                reloaded = {k[len('module.'):]: v for k, v in reloaded.items()}
            reloaded['embeddings.weight'] = model.state_dict()['embeddings.weight']
        ...
        decoder = TransformerModel(params, tgt_dico, is_encoder=False, with_output=True)
        ...
        if params.reload_mt_and_enc_emb:
            assert params.reload_emb == '' and params.reload_model == '', 'Cannot set other reloading flags with --reload_mt_and_enc_emb'
            mt_path, bert_path = params.reload_mt_and_enc_emb.split(',')
            assert mt_path != '' and bert_path != ''

            logger.info("Reloading MT from %s and encoder embeddings from BERT %s ..." % (mt_path, bert_path))
            bert_reload = torch.load(bert_path, map_location=lambda storage, loc: storage.cuda(params.local_rank))['model']
            if all([k.startswith('module.') for k in bert_reload.keys()]):
                bert_reload = {k[len('module.'):]: v for k, v in bert_reload.items()}
            
            mt_reload = torch.load(mt_path, map_location=lambda storage, loc: storage.cuda(params.local_rank))
            enc_reload = mt_reload['encoder']
            if all([k.startswith('module.') for k in enc_reload.keys()]):
                enc_reload = {k[len('module.'):]: v for k, v in enc_reload.items()}
            enc_reload['embeddings.weight'] = bert_reload['embeddings.weight']
            enc_reload['pred_layer.proj.weight'] = bert_reload['pred_layer.proj.weight']
            enc_reload['pred_layer.proj.bias'] = bert_reload['pred_layer.proj.bias']
            encoder.load_state_dict(enc_reload)
            
            dec_reload = mt_reload['decoder']
            if all([k.startswith('module.') for k in dec_reload.keys()]):
                dec_reload = {k[len('module.'):]: v for k, v in dec_reload.items()}
            decoder.load_state_dict(dec_reload)
