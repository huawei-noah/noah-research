...
def set_tgt_dico_parameters(params, data, dico):
    """
    Update dictionary parameters.
    """
    if 'tgt_dico' in data:
...
def load_para_data(params, data):
    ...
    for src, tgt in params.para_dataset.keys():
        ...
        for splt in ['train', 'valid', 'test']:
            ...
            # update dictionary parameters
            if (src, tgt) in params.mt_steps:
                set_dico_parameters(params, data, src_data['dico'])
                set_tgt_dico_parameters(params, data, tgt_data['dico'])
            else:
                set_dico_parameters(params, data, tgt_data['dico'])
                set_tgt_dico_parameters(params, data, src_data['dico'])
